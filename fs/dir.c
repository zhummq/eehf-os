...
bool search_dir_entry(struct partition *part, struct dir *pdir, const char
*name, struct dir_entry *dir_e)
{
...
p_de为指向目录项的指针,值为buf起始地址 uint32_t dir_entry_size =
part->sb->dir_entry_size; uint32_t dir_entry_cnt = SECTOR_SIZE / dir_entry_size;
// 1扇区内可容纳的目录项个数
...
此时p_de已经指向扇区内最后一个完整目录项了,需要恢复p_de指向为buf memset(buf, 0,
SECTOR_SIZE);    // 将buf清0,下次再用
    }
    sys_free(buf);
    sys_free(all_blocks);
    return false;
}*/
static bool buf_find(struct partition *part, uint32_t *i_sectors,
                     const char *name, struct dir_entry *dir_e, void *buf,
                     uint32_t dir_entry_size, uint32_t index) {
...
bool search_dir_entry(struct partition *part, struct dir *pdir,
  ...
  if (buf_find(part, pdir->inode->i_sectors, name, dir_e, buf, dir_entry_size,
               12)) {
    sys_free(lba_buf);
    sys_free(double_buf);
    return TRUE;
...
/*
bool sync_dir_entry(struct dir *parent_dir, struct dir_entry *p_de, void
*io_buf)
{
...
每扇区最大的目录项数目 int32_t block_lba = -1;
...
dir_e用来在io_buf中遍历目录项 int32_t block_bitmap_idx = -1;
...
将上面分配的块做为一级间接块表地址 block_lba = -1; block_lba =
block_bitmap_alloc(cur_part); // 再分配一个块做为第0个间接块 if (block_lba ==
-1)
                {
                    block_bitmap_idx = dir_inode->i_sectors[12] -
cur_part->sb->data_start_lba; bitmap_set(&cur_part->block_bitmap,
block_bitmap_idx, 0); dir_inode->i_sectors[12] = 0; printk("alloc block bitmap
for sync_dir_entry failed\n"); return false;
                }
...
static bool buf_write(struct dir_entry *p_de, uint32_t index, void *io_buf,
                      uint32_t *i_sectors, uint32_t dir_entry_size) {
  for (uint32_t i = 0; i < index; i++) {
    uint32_t lba = i_sectors[i];
    if (lba == 0) {

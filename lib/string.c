...
void memset(void *dst_, uint8_t value, uint32_t size) {
  ...
  while (
      size-- >
      0) // 先判断size是否>0，然后再减，然后执行循环体，size是多少，就会循环多少次
    *dst++ =
        value; //*的优先级高于++，所以是先对dst指向的地址进行操作(*dst=value)，然后地址+1
}

// 将src地址起始处size字节的数据移入dst，用于拷贝内存数据
// src起始是有数据的，所以用const
// void*，const修饰void*，意味着地址内的数据是只读
...
uint32_t strlen(const char *str) {
  ...
  const char *p = str;
  while (*p++)
    ; // 1、先取*p的值来进行2的判断     2、判断*p,决定是否执行循环体
      // 3、p++(这一步的执行并不依赖2的判断为真)
...
void *calloc(uint32_t nmemb, uint32_t size) { return malloc(nmemb * size); }

...
struct ide_channel ide_channels[2]; // 有两个ide通道
...
/* 从硬盘读取sec_cnt个扇区到buf */
static void ide_read_one(struct disk *hd, uint32_t lba, void *buf,
                         uint32_t sec_cnt) {
  ASSERT(lba <= max_lba);
  ASSERT(sec_cnt > 0);
...
static void ide_write_one(struct disk *hd, uint32_t lba, void *buf,
                          uint32_t sec_cnt) {
  ASSERT(lba <= max_lba);
  ASSERT(sec_cnt > 0);
  lock_acquire(&hd->my_channel->lock);
...
void intr_hd_handler(uint8_t irq_no) {
  ...
  struct ide_channel *channel = &ide_channels[ch_no];
...
void ide_init() {
  ...
  while (channel_no < channel_cnt) {
    channel = &ide_channels[channel_no];

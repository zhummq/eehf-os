...
static void e1000_eeprom_detect(void){
  volatile uint32_t * base = ( volatile uint32_t *)e1000.dev->bar[0].iobase;
  base = base + 0x14; // read addr
  *base = 0x1;
...
static uint16_t e1000_eeprom_read(uint8_t addr){
  volatile uint32_t * base =(volatile uint32_t *) e1000.dev->bar[0].iobase + 0x14;
  uint32_t tmp;
  if (e1000.eeprom){
...
void e1000_init(void){
  ...
  if (bar->index < 0){
    ...
    printk("%x   %x",bar->iobase,bar->size);
  map_area(bar->iobase,bar->size); 

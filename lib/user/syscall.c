...
/* 创建目录pathname */
int32_t mkdir(const char *pathname, uint32_t mode) {
...
char get_keycode(void) { return _syscall0(SYS_GET_KEYCODE); }
void draw_piexl(uint32_t x, uint32_t y, uint32_t color) {
  _syscall3(SYS_DRAW_PIEXL, x, y, color);
}
void msleep(uint32_t ms) { _syscall1(SYS_MSLEEP, ms); }
uint32_t get_ms(void) { return _syscall0(SYS_GET_MS); }
void *realloc(void *ptr, uint32_t size) {
  return (void *)_syscall2(SYS_REALLOC, ptr, size);
}
void cursor(void) { _syscall0(SYS_CURSOR); }

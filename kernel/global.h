...
#define GDT_ATTR_HIGH                                                          \
  ((DESC_G_4K << 7) + (DESC_D_32 << 6) + (DESC_L << 5) +                       \
   (DESC_AVL << 4)) // 定义段描述符的高32位的高字
#define GDT_CODE_ATTR_LOW_DPL3                                                 \
  ((DESC_P << 7) + (DESC_DPL_3 << 5) + (DESC_S_CODE << 4) +                    \
   DESC_TYPE_CODE) // 定义用户程序用的代码段描述符高32位的低字
#define GDT_DATA_ATTR_LOW_DPL3                                                 \
  ((DESC_P << 7) + (DESC_DPL_3 << 5) + (DESC_S_DATA << 4) +                    \
   DESC_TYPE_DATA) // 定义用户程序用的数据段描述符高32位的低字

//---------------  TSS描述符属性  ------------
#define TSS_DESC_D                                                             \
  0 // 这个D/B位在其他段描述中用于表示操作数的大小，但这里不是，实际上它根本就没有被使用（总是设置为0）。
...
#define DIV_ROUND_UP(X, STEP)                                                  \
  ((X + STEP - 1) / (STEP)) // 用于向上取整的宏，如9/10=1

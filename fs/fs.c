...
static void partition_format(struct partition *part) {
  ...
  uint32_t block_bitmap_sects =
      DIV_ROUND_UP(now_total_free_sects, BITS_PER_SECTOR); // 初始估算
  ...
  sb.data_start_lba = sb.inode_table_lba +
                      sb.inode_table_sects; // 数据区的起始就是inode数组的结束
  sb.root_inode_no = 0;
  ...
  uint32_t block_bitmap_last_byte =
      block_bitmap_bit_len / 8; // 计算出块位图最后一字节的偏移
  uint8_t block_bitmap_last_bit =
      block_bitmap_bit_len % 8; // 计算出块位图最后一字节中有效位的数量
  uint32_t last_size =
      SECTOR_SIZE -
...
int32_t sys_mkdir(const char *pathname, uint32_t mode) {
  ...
  struct path_search_record searched_record;
  memset(&searched_record, 0, sizeof(struct path_search_record));
  char *new_pathname = sys_getcwd(io_buf, SECTOR_SIZE * 2);
  strcat(new_pathname, pathname);
  int inode_no = -1;
  inode_no = search_file((const char *)new_pathname, &searched_record);
...
/* 向屏幕输出一个字符 */
void sys_putchar(char char_asci) { put_char_bga(char_asci); }
